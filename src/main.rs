...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Watch {
            ...
            // Watch previously used an AST-only rebuild that never loaded
            // `graphify-rs.toml`: doc/paper files got no semantic extraction, so
            // every rebuild overwrote a full graph with a code-only one. Resolve
            // the same settings `build` does and reuse its pipeline.
            let app_cfg = config::load_config(Path::new(&path));
            let out_dir = match output {
                Some(o) => o,
                None => app_cfg.output.clone().unwrap_or_else(|| {
            ...
            let effective_formats = app_cfg.formats.clone().unwrap_or_default();
            let llm_cfg = app_cfg.llm.clone();
            let jobs = cli.jobs;
            let handle = tokio::runtime::Handle::current();

            let rebuild_fn: graphify_watch::RebuildFn = std::sync::Arc::new(
                move |root: &Path, out: &Path, _changed: Option<&[PathBuf]>| {
                    // Runs on a spawn_blocking thread, so block_on is safe here
                    // (never called from a runtime worker thread).
                    handle
                        .block_on(cmd_build::cmd_build(
            ...
            graphify_watch::watch_directory_with(Path::new(&path), Path::new(&out_dir), rebuild_fn)
                .await?;
        }
